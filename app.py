"""Placement readiness dashboard for students and placement teams."""
...
# Statistical tests used in the analytics tabs.
from scipy import stats as sps

# Data preparation and basic modelling tools.
...
# Demo users for the Streamlit session.
USERS_DATABASE = {
    "admin": {
...
def load_company_dataset() -> pd.DataFrame:
    """Return the sample company list used for matching and filtering."""
    companies_data = {
        "Company": [
...
def generate_student_dataset(n: int = 400, seed: int = 42) -> pd.DataFrame:
    """Create a repeatable sample student dataset for the analytics screens."""
    ...
    # Hidden score used to make the sample placements feel realistic.
    z = (
        (cgpa - 6.5) * 1.1
        - backlogs * 0.6
        + internships * 0.5
    ...
    # Add a few messy values so the wrangling screen has something to fix.
...
# Options shown in the student profile form.
...
def calculate_employability_score(cgpa, backlogs, prog_langs, tech_skills,
                                  ...
                                  company_alignment):
    """Calculate a simple readiness score from academic and skill inputs."""
...
def get_eligible_companies(cgpa: float, backlogs: int, student_skills: list,
    ...
    """Find companies where the student clears the basic criteria."""
...
def render_login():
    ...
    st.markdown(
        ...
        'Placement readiness, skill gaps, data exploration and company matching'
        '</div>', unsafe_allow_html=True
    )
    st.markdown(
        '<div style="text-align:center;">'
        '<span class="fda-chip">Student Profile</span>'
        '<span class="fda-chip">Skill Gap</span>'
        '<span class="fda-chip">Data Lab</span>'
        '<span class="fda-chip">ML Lab</span>'
        '<span class="fda-chip">Companies</span>'
...
def tab_student_profile(companies_df: pd.DataFrame):
    ...
    # Average match across eligible companies gives a rough alignment score.
...
def tab_data_lab():
    st.markdown('<div class="section-title">🧪 Data Lab — Pandas / NumPy</div>',
                unsafe_allow_html=True)
    st.caption("Upload a dataset or use the sample data to explore it quickly.")
...
def tab_wrangling_lab():
    st.markdown('<div class="section-title">🧹 Data Wrangling Lab</div>',
                unsafe_allow_html=True)
    st.caption("Clean missing values, outliers, scaling, encoding and PCA in one place.")
...
def tab_eda():
    st.markdown('<div class="section-title">📊 Exploratory Data Analysis</div>',
                unsafe_allow_html=True)
    st.caption("Summary tables, distributions, relationships and placement trends.")
...
def tab_ml_lab():
    st.markdown('<div class="section-title">🤖 Machine Learning Lab</div>',
                unsafe_allow_html=True)
    st.caption("Train a placement predictor and group similar students with clustering.")
    ...
    with tab_unsup:
        ...
        # Project clusters into two dimensions so they are easier to read.
...
def tab_stats_lab():
    st.markdown('<div class="section-title">📐 Statistical Foundations</div>',
                unsafe_allow_html=True)
    st.caption("Descriptive statistics, hypothesis tests and correlation checks.")
...
def main():
    ...
    st.markdown(
        ...
        'Placement Analytics · Built with Streamlit · '
